...
증분은 기계에 맡길 수 있게 싸지고, 두 기계가 같은 기억을 갖는 순간은 사람이 쥔다.
그리고 그 결정을 대문과 담당자 문서가 처음으로 같은 말로 적는다.

### 세션 sync — `--local` / `--global`

- **기본값이 `--local`이 됐다.** 이 기기의 라이브 세션만 색인하고 **ssh를 한 번도
  쓰지 않으며 복제본을 건드리지 않는다.** 타이머로 돌릴 만큼 싸고, systemd 유닛은
  이미 리포에 있다(설치는 안 됐다).
- **`--global`은 한 act다** — 전 기기 `gather --strict` → embed → verify →
  인덱스·매니페스트·**코퍼스**를 함께 publish. 셋을 묶은 이유는 실측이다: 09-03에
  양 트랙이 각각 오라클에 **소스 없는 인덱스**를 남겼고, 오라클 orphan 7건이 정확히
  "인덱스는 갔는데 코퍼스가 gather보다 뒤처진" 모양이었다. INVARIANT 6.6이 DB와
  매니페스트를 묶었듯, 이제 코퍼스까지 같이 간다.
- `--push`는 deprecated alias로 남긴다.
- **자동화는 하지 않는다(GLG).** 다만 이건 *거절*이 아니라 **범위 결정**이다 —
  잦은 증분은 기계가, *정합의 순간*은 사람이. GLG가 그날 댄 이유는 정합이 아니라
  =오는 것= 이었고 =당분간= 이 붙어 있었다. 날짜가 박힌 결정이지 영구 원리가 아니다.

### 스킬 문서 — 정본이 움직일 때 사본이 뒤에 남는다

- `andenken-embed` SKILL.md가 **오퍼레이터가 실제로 하는 부탁**으로 시작하도록
  머리를 바꿨다. 라우팅 표가 계약이다(1 노트북 / 2 글로벌 / 3 가든 / 4 OpenClaw).
- `corpus:replicate`를 push 시퀀스 **안에** 넣었다. 밖에 적힌 절차는 안 불린다.
- **리플리카의 orphan은 리플리카 탓이 아니라 소스가 뒤처졌다는 신호**라고 적었다.
  진단을 결과가 아니라 원인 쪽에 두는 문장.
- **문서만 읽은 테스터를 불러 실제로 치게 했더니 구멍 다섯이 나왔다.** 표를 복사해
  실행하니 없는 명령이었고, "네트워크 없음"이 오프라인으로 읽혔다. 자동화가 없는
  설계에서 **요청과 명령 사이에 서 있는 것은 문서 하나뿐**이므로, 문서의 소유는 곧
  검증 의무다.
- 같은 날 결함 둘이 더 나왔고 둘 다 같은 과였다. `--help`의 **고정 줄번호**
  (`sed -n '2,32p'`)는 헤더가 자라자 Usage를 잘라먹었다 — 범위를 줄번호가 아니라
  **코멘트 블록 끝에 앵커**했다. 스킬 아래쪽 네 절은 상단을 고친 뒤에도 계속
  `--push`를 권하고 있었다. **일곱 중 다섯을 남이 잡았다**(형제 둘, 테스터 하나).

### 문서 — 대문과 담당자 문서를 코퍼스 시대로

- **README**: 맨 앞에 `## Who owns the memory`를 세웠다. DB가 주인인 가족
  (beads·Letta·Hermes)의 정반대 꼭짓점이라는 포지션과, 그것이 이 리포에서 눈에
  보이는 형태 셋(자동으로 꿈꾸지 않는다 / 기억이 기계가 아니라 사람을 따라간다 /
  문서의 소유는 검증 의무). 공동 레퍼런스
  [botlog 20260408T120252](https://notes.junghanacs.com/botlog/20260408T120252.html)로
  링크 — andenken·agent-config·dictcli·nixos-config·aionsclubs가 각자 자리에서
  같은 질문에 답한 문서다.
- **README 아키텍처 도해의 사실 오류 수정**: `Temporal Decay`가 여전히 기능처럼
  적혀 있었다. 두 라이브 트랙 모두 `recencyHalfLifeDays: 0`이다
  (`index.ts` · `cli.ts` 세션 분기 · `md-search.ts`). 왜 껐는지(보통 hit 약 49일,
  강한 hit 약 85일에 `minScore` 아래로 사라짐 = 랭킹 신호의 탈을 쓴 하드 삭제)와
  최근성 의도가 `--mode recent`로 갔다는 것을 본문에 적었다.
- **README 세션 소스절을 코퍼스로 다시 썼다** — 라이브 경로 앞에 device 한 마디를
  덧댄 레이아웃이 계약이라는 것, 추가 전용·실복사 두 규칙, 왜 필요했는지의 실측
  (oracle 고유 455건 / 0.69GB, openclaw workspace 64건은 노트북에 한 건도 없음),
  git 대신 체크섬 매니페스트를 고른 이유(806MB 팩 + gitleaks 47분 47초), 그리고
  `MANIFEST.sha256`은 **andenken 없이 검증된다**는 것.
- **README 운영절**에 `sync:sessions --local|--global` · `corpus:gather` ·
  `corpus:manifest verify` · `accept`를 세우고 `ANDENKEN_INDEX_AUTHORITY` 게이트를
  설명했다. 수치를 현재값으로 갱신(sessions 76,044 / 1,627 files,
  md 10,704 / 2,230 files). milestone에 09-03·09-02·08-10을 추가.
- **AGENTS.md**: §Session corpus sources를 코퍼스 계약으로 교체(`MANIFEST.json`
  SSOT · `DEVICES.json` roster 분리 이유 · gather가 실패하면 색인을 거부),
  endpoints 표의 sessions 행, `sync-sessions.sh` 운영절, cadence절을 갱신.
  cadence절은 authority 게이트가 **push가 아니라 색인 진입**을 막는다는 것과
  왜 그래야 하는지(push만 막으면 리플리카가 스스로를 포크한다 — 2026-06-19→07-06,
  27,966 vs 24,882)를 적는다. Pointers에 CHANGELOG와 기억 주인 레퍼런스 추가.

### 이슈

- **#10·#11 닫힘.** #10의 마지막 갭은 처방이 뒤집혔다 — "코퍼스 밖"이 아니라
  **JSONL이 없다**(sqlite 이관). 300KB 플로어는 모순이 아니었고, 가드 창은 이미
  닫혀 있었다.
- **#13 신설 — OpenClaw 기억면 회수.** OpenClaw가 6봇 세션을 이미
  `qwen/qwen3-embedding-8b` 4096d로 임베딩해 뒀다. **같은 모델·같은 차원이라
  재임베딩 없이 import**하면 된다(약 80MB, API 0원). 코드는 아직 한 줄도 없다.

### 실측

- 양쪽 최종 상태: sessions **76,044 chunks / 1,627 files**, md **10,704 / 2,230**,
  코퍼스 2,181 files / 3.0G, 양쪽 verify 통과. 09-03 임베딩 총비용 약 $0.005.
