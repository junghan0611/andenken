...
// --- MD Manifest (mirrors session/org pattern; same shape, different path) ---
//
// md tracks the exported public garden (~/repos/gh/notes/content). Files are
// regenerated by the Hugo export pipeline rather than appended in place, so
// mtime-based stale detection is the natural primitive: a re-exported file
// gets a new mtime even when content barely changed, and that's fine — the
// chunker is fast and the embedding cost per file is small.
type MdFileManifest = OrgFileManifest;

function getMdManifestPath(): string {
  return path.join(getDataDir(), "md-manifest.json");
}

function loadMdManifest(): MdFileManifest {
  const p = getMdManifestPath();
  try {
    return JSON.parse(fs.readFileSync(p, "utf-8"));
  } catch {
    return { files: {}, lastUpdated: "" };
...
function saveMdManifest(manifest: MdFileManifest): void {
  manifest.lastUpdated = new Date().toISOString();
  fs.writeFileSync(getMdManifestPath(), JSON.stringify(manifest, null, 2));
}
...
/**
 * MD-track provider for the CLI indexer (Issue #8).
 *
 * Reads ANDENKEN_MD_* exclusively. No legacy fallback — md is a new track
 * and must never pick up the org-shaped vLLM env by accident. Recommended
 * provider is OpenRouter qwen/qwen3-embedding-8b 4096d, same as sessions.
 */
function getMdProvider(): EmbeddingProvider {
  const p = createMdProviderFromEnv();
  if (!p) {
    throw new Error(
      "No md embedding provider available. Set ANDENKEN_MD_PROVIDER " +
      "(and ANDENKEN_MD_ENDPOINT / _MODEL / _DIMENSIONS / _API_KEY).",
    );
  }
  console.log(`📡 MD provider: ${p.name} (${p.dimensions}d)`);
  return p;
}
...
// --- MD Indexing (public garden direct embedding) ---
//
// Operates on `~/repos/gh/notes/content` exported Markdown via md-chunker.
// Provider: ANDENKEN_MD_* (recommended: OpenRouter Qwen3-Embedding-8B 4096d).
// Store: data/md.lance — separate LanceDB from sessions and org.
// Manifest: data/md-manifest.json — mtime/size driven incremental.
//
// Same paid-remote --force gate as sessions: full md rebuilds against a
// paid endpoint require ANDENKEN_ALLOW_PAID_FULL_REBUILD=1.
...
async function indexMd(force: boolean) {
  ...
  const store = new VectorStore(getMdDbPath(), provider.dimensions || 4096);
  await store.init();
  if (force) await store.reset();
  await store.ensureTable();
  await store.assertCompatibleDim();
  ...
  const indexed = force ? new Set<string>() : await store.getIndexedFiles();
  let manifest: MdFileManifest = force
    ? { files: {}, lastUpdated: "" }
    : loadMdManifest();
  const hasManifest = Object.keys(manifest.files).length > 0;
  ...
  if (!hasManifest && !force) {
    ...
    console.log(`📌 Initializing md manifest from ${baselined} indexed files (baseline)`);
  ...
  const tasks = toIndex.map((file) => async () => {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[md-index-error] ${file} :: ${msg}`);
      progress.error();
      throw err;
    }
  });

  const { errors: mdErrors } = await runWithConcurrency(tasks, CONCURRENCY);
  await wb.flush();
  saveMdManifest(manifest);
  if (mdErrors > 0) {
    throw new Error(`MD indexing failed for ${mdErrors} files`);
  }
  ...
  console.log(progress.summary());
  console.log(
    `💰 API: ${stats.calls} calls, ~${(stats.estimatedTokens / 1000).toFixed(0)}K tokens, ~$${stats.estimatedCostUSD.toFixed(3)}`,
  );
  console.log(`Total in DB: ${total}`);
  await store.close();
}
...
async function collectMdStatus(): Promise<StatusTrack> {
  ...
  let indexedFiles = new Set<string>();
  if (exists) {
    const store = new VectorStore(dbPath, 4096);
    await store.init();
    count = await store.getCount();
    actualDim = await store.getActualVectorDim();
    indexedFiles = await store.getIndexedFiles();
...
async function status() {
  ...
  // MD track — issue #8. Mirrors the sessions block.
  if (md.exists) {
    const mDbPath = getMdDbPath();
    const mSize = execSync(`du -sh ${mDbPath}`).toString().split("\t")[0];
    const mFragDir = path.join(mDbPath, "session_chunks.lance", "data");
    const mFrags = fs.existsSync(mFragDir) ? fs.readdirSync(mFragDir).length : 0;
    const mDimLabel = md.actual_dim
      ? `${md.actual_dim}d`
      : mdProviderDim
        ? `empty, provider=${mdProviderDim}d`
        : "empty";
    console.log(
      `📝 MD (${mDimLabel}): ${md.count} chunks | ${md.indexed_files}/${md.files} files | ${mFrags} frags | ${mSize}`,
    );
    if (md.manifest_entries > 0) {
      console.log(
    ...
    } else if (md.indexed_files > 0) {
      console.log(
        `   ↳ manifest: empty (will baseline on next index:md)`,
      );
    }
    if (md.actual_dim && mdProviderDim && md.actual_dim !== mdProviderDim) {
      console.log(
        `   ⚠ md provider dim=${mdProviderDim}d differs from DB dim=${md.actual_dim}d — search/index paths will refuse until rebuild or config fix`,
      );
    }
  } else {
    console.log("📝 MD: not indexed yet");
  }
