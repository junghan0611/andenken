#!/usr/bin/env bash
# DEPRECATED — Qwen3-Embedding-4B 2560d sessions sync (ollama or gpu1i).
#
# This file is the PR-A-era sync-sessions.sh, preserved verbatim. PR-B
# replaces the default sessions track with OpenRouter Qwen3-Embedding-8B
# (4096d). The new sessions DB schema is INCOMPATIBLE with this script —
# running it against a 4096d sessions.lance corrupts the index.
#
# Use `scripts/sync-sessions.sh` (4096d, OpenRouter) for normal operation.
# This script remains only so an operator can briefly fall back to the
# local 4B path during cutover or debugging — and only after destroying
# data/sessions.lance and rebuilding from scratch on 2560d.
#
# DO NOT run from cron / hourly automation. PR-B operating posture is
# 4096d only. See ROADMAP.md "변화 기록" for context.
#
# --- original PR-A header (unchanged below) ---
...
#
# Usage (legacy, 2560d only):
#   ./scripts/sync-sessions-legacy-4b.sh             # sessions-only, no oracle push
#   ./scripts/sync-sessions-legacy-4b.sh --push      # also rsync to oracle
#   ./scripts/sync-sessions-legacy-4b.sh --backend ollama
#   ./scripts/sync-sessions-legacy-4b.sh --backend gpu1i
set -euo pipefail
