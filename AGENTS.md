...
| **OpenClaw** | `qwen/qwen3-embedding-8b` / 4096d — **embedded by OpenClaw, not by us** | `ANDENKEN_OPENCLAW_HOST` (default `oracle`), `sqlite3 -readonly` snapshot of each bot's `openclaw-agent.sqlite` → `data/openclaw.lance` | explicit call only: `./run.sh sync:openclaw` (delta by per-agent watermark, `--full` from zero). **API 0** — the vectors come already computed. Local only; no replication. |
